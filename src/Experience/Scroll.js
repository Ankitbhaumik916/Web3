...
class Scroll {
  ...
  bindEvents() {
    ...
    // Modified onWheel that checks if we're in gallery section
    const onWheelWithCheck = (event) => {
      const scrollPos = window.scrollY
      const galleryTop = gallerySection?.offsetTop || Infinity
      const contactTop = contactSection?.offsetTop || Infinity
      
      // Only prevent default and handle scroll if we're in gallery or contact section
      if (scrollPos + window.innerHeight >= galleryTop && scrollPos < contactTop) {
        event.preventDefault()
        const normalizedWheelDelta = this.normalizeWheelDelta(event) * this.wheelScrollSpeed
        this.addScrollInput(normalizedWheelDelta)
      }
    ...
    // Modified touch handlers that check position
    const onTouchStartWithCheck = (event) => {
      this.touchY = event.touches[0]?.clientY ?? 0
    }
    
    const onTouchMoveWithCheck = (event) => {
      const scrollPos = window.scrollY
      const galleryTop = gallerySection?.offsetTop || Infinity
      const contactTop = contactSection?.offsetTop || Infinity
    ...
    window.addEventListener('wheel', onWheelWithCheck, { passive: false })
    window.addEventListener('touchstart', onTouchStartWithCheck, { passive: true })
    window.addEventListener('touchmove', onTouchMoveWithCheck, { passive: false })
  }
